...
import { readTableDDL } from './pgDdlReader'
...
export class PostgresDriver implements IDriver {
  ...
  async getObjectDDL(
    ...
    return readTableDDL(this.client, schemaName, objectName)
  }
  ...
  async getRoles(): Promise<string[]> {
    const result = await this.client.query(`SELECT rolname FROM pg_roles ORDER BY rolname`)
    return result.rows.map((r) => r.rolname as string)
  }

  async createSchema(schemaName: string, owner?: string): Promise<void> {
    const q = (s: string) => '"' + s.replace(/"/g, '""') + '"'
    let sql = `CREATE SCHEMA ${q(schemaName)}`
    if (owner) sql += ` AUTHORIZATION ${q(owner)}`
    await this.client.query(sql)
  }

  async getSchemaOwner(schemaName: string): Promise<string> {
    const result = await this.client.query(
  ...
  async alterSchema(schemaName: string, newName?: string, newOwner?: string): Promise<void> {
    const q = (s: string) => '"' + s.replace(/"/g, '""') + '"'
    let currentName = schemaName
    if (newName && newName !== schemaName) {
      await this.client.query(`ALTER SCHEMA ${q(currentName)} RENAME TO ${q(newName)}`)
      currentName = newName
    }
    if (newOwner) {
      await this.client.query(`ALTER SCHEMA ${q(currentName)} OWNER TO ${q(newOwner)}`)
    }
  }

  async dropSchema(schemaName: string, cascade: boolean): Promise<void> {
    const q = (s: string) => '"' + s.replace(/"/g, '""') + '"'
    await this.client.query(`DROP SCHEMA ${q(schemaName)} ${cascade ? 'CASCADE' : 'RESTRICT'}`)
  }
  ...
  async alterView(schemaName: string, viewName: string, newViewName?: string, newSelectQuery?: string): Promise<void> {
    ...
    try {
      ...
      if (newViewName && newViewName !== viewName) {
        await this.client.query(
          `ALTER VIEW ${q(schemaName)}.${q(viewName)} RENAME TO ${q(newViewName)}`
        )
      }
      await this.client.query('COMMIT')
    } catch (err) {
  ...
  async dropView(schemaName: string, viewName: string, cascade: boolean): Promise<void> {
    const q = (s: string) => '"' + s.replace(/"/g, '""') + '"'
    await this.client.query(`DROP VIEW ${q(schemaName)}.${q(viewName)}${cascade ? ' CASCADE' : ''}`)
  }

  async createIndex(params: CreateIndexParams): Promise<void> {
    const q = (s: string) => '"' + s.replace(/"/g, '""') + '"'
    const colList = params.columns.map((c) => `${q(c.name)} ${c.order}`).join(', ')
    const sql =
      `CREATE ${params.unique ? 'UNIQUE ' : ''}INDEX ${q(params.indexName)}` +
      ` ON ${q(params.schemaName)}.${q(params.tableName)}` +
      ` USING ${params.method} (${colList})`
    await this.client.query(sql)
  }

  async dropIndex(schemaName: string, indexName: string): Promise<void> {
    const q = (s: string) => '"' + s.replace(/"/g, '""') + '"'
    await this.client.query(`DROP INDEX ${q(schemaName)}.${q(indexName)}`)
  }
