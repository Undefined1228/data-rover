<script lang="ts">
  ...
  let { schemaName, currentOwner }: { schemaName: string; currentOwner: string } = $props()

  const IDENT_RE = /^[a-zA-Z_][a-zA-Z0-9_$]*$/

  let newName = $state(schemaName)
  ...
  function validate(): boolean {
    if (!newName.trim() || !IDENT_RE.test(newName)) {
      nameError = true
      return false
    }
    return true
  ...
  async function handleSave(): Promise<void> {
    ...
    error = null
    try {
      const data = await api.request<{ ok: boolean; message?: string }>('schema:alter', {
        schema: schemaName,
        newName: newName !== schemaName ? newName : undefined,
...
<div class="dialog-root">
  <div class="dialog-header">
    <h2>스키마 편집</h2>
    <p>"{schemaName}" 스키마의 이름과 소유자를 변경합니다.</p>
  </div>
  ...
  <div class="dialog-footer">
    <button class="btn btn-primary" onclick={handleSave} disabled={saving || loadingRoles}>
      {saving ? '저장 중...' : '저장'}
    </button>
  </div>
</div>
